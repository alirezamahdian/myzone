...
public class MyZoneEngine{
    ...
    public MyZoneEngine(String prefix){
        ...
        for (int i = 0; i < mainSettings.friends.size(); i++){
            if (mainSettings.friends.get(i).getStatus().equals("connected"))
                mainSettings.friends.get(i).setStatus("disconnected");
        }
    ...
    public boolean start(){
        ...
        while (disconnected){
            try{
                ...
                while(!disconnected){
                    try{
                        for (int i = mainSettings.pendingFriendships.size() - 1; i >= 0; i--){
